    ...
    <main class="mx-auto flex w-full max-w-7xl flex-col gap-6 px-5 py-6 sm:px-8 lg:px-10">
        <header class="rounded-xl border border-slate-200 bg-white/95 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/95">
            <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
                <div>
                    <p class="text-sm font-medium uppercase tracking-[0.18em] text-[#1e4f91] dark:text-blue-400">{{ __('Panel del empleado') }}</p>
                    <h1 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950 dark:text-slate-100">{{ __('Mi asistencia') }}</h1>
                    <p class="mt-1 max-w-2xl text-sm text-slate-500 dark:text-slate-400">{{ __('Consulta tus registros recientes de entrada y salida.') }}</p>
                </div>
                ...
                <div class="flex items-center gap-3 rounded-xl border border-[#0f2f5f]/10 bg-[#0f2f5f]/5 px-4 py-2.5 shadow-xs dark:border-blue-900/40 dark:bg-blue-950/40">
                    ...
                    <div class="flex flex-col">
                        <span class="text-xs font-semibold uppercase tracking-wider text-[#1e4f91] dark:text-blue-300">{{ __('Empleado') }}</span>
        ...
        <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="border-b border-slate-200 px-6 py-4 dark:border-slate-800">
                <h2 class="text-base font-semibold text-slate-950 dark:text-slate-100">{{ __('Últimos registros') }}</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Historial ordenado desde el movimiento más reciente.') }}</p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse ($attendances as $attendance)
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                                ...
                                <td class="px-6 py-4 text-slate-700 dark:text-slate-300">{{ $attendance->occurred_at->format('Y-m-d') }}</td>
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="mx-auto max-w-sm">
                                        <div class="mx-auto flex size-12 items-center justify-center rounded-full bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                                            <flux:icon.clock class="size-6" />
                                        </div>
                                        <p class="mt-3 font-medium text-slate-900 dark:text-slate-100">{{ __('Aún no hay registros de asistencia.') }}</p>
                                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Cuando escanees el QR de tu sección, tus movimientos aparecerán aquí.') }}</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
