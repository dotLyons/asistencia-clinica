<main class="mx-auto flex w-full max-w-7xl flex-col gap-6 px-5 py-6 sm:px-8 lg:px-10">
    <header class="rounded-xl border border-slate-200 bg-white/95 p-6 shadow-sm">
        <div>
            <p class="text-sm font-medium uppercase tracking-[0.18em] text-[#1e4f91]">{{ __('Administración') }}</p>
            <h1 class="mt-2 text-2xl font-semibold tracking-tight text-slate-950">{{ __('Gestión de Secciones') }}</h1>
            <p class="mt-1 max-w-2xl text-sm text-slate-500">{{ __('Crea y administra las secciones del establecimiento. Cada sección tiene su propio código QR de asistencia en formato JPG.') }}</p>
        </div>
    </header>

    <div class="grid gap-6 lg:grid-cols-[380px_1fr]">
        <!-- Create Section Form -->
        <section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm h-fit">
            <h2 class="text-base font-semibold text-slate-950 mb-4">{{ __('Crear Nueva Sección') }}</h2>
        ...
        <section class="flex flex-col gap-4">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-slate-200 bg-white/95 p-4 shadow-sm">
                                ...
                                <tr class="hover:bg-slate-50/80">
                                    ...
                                    <td class="px-6 py-4 text-right space-x-1 whitespace-nowrap">
                                        <flux:button 
                                            ...
                                            class="cursor-pointer bg-[#0f2f5f] text-white hover:bg-[#173f7a]"
